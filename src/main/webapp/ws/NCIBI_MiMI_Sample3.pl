...
my $response = $ua->get('http://mimi.ncibi.org/MimiWeb/fetch.jsp?gene=1436&type=reactions');

my $xp = XML::XPath->new(xml => $response->content);

foreach my $reactionid ($xp->find('//PUT_SOMETHING_HERE')->get_nodelist) {
	print $reactionid->string_value . "\n";
}
